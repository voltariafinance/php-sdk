...
```bash
composer require voltaria/voltaria-sdk
```

Construct a client with your API key and make a read call. The environment is selected automatically from the key prefix:
...
// Production
$client = new Voltaria\Client("live_...");

// Sandbox
// $client = new Voltaria\Client("sandbox_...");

$loans = $client->loans->listLoans();
```
...
Passing explicit options (such as a base URL) via `ClientOptions` overrides the prefix-based routing.

See [reference.md](./reference.md) for the full API reference.

---

[![fern shield](https://img.shields.io/badge/%F0%9F%8C%BF-Built%20with%20Fern-brightgreen)](https://buildwithfern.com?utm_source=github&utm_medium=github&utm_campaign=readme&utm_source=Voltaria%2FPHP)
[![php shield](https://img.shields.io/badge/php-packagist-pink)](https://packagist.org/packages/voltaria/voltaria-sdk)

The Voltaria PHP library provides convenient access to the Voltaria APIs from PHP.
...
try {
    $response = $client->clients->createClient(...);
} catch (VoltariaApiException $e) {
...
$client = new VoltariaClient(options: [
    'client' => $customClient
]);
